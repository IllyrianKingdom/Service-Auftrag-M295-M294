...
function getAllBerichte($conn) {
  try {
    ...
    $berichte = array_map(function($b) {
      return transformBerichtToFrontend($b);
    }, $berichte);
...
function createBericht($conn) {
  ...
  if ($arbeitszeit <= 0) {
    sendError(400, 'Arbeitszeit muss größer als 0 sein');
  }

  if (!isValidDate($data['arbeitsdatum'])) {
    sendError(400, 'Ungültiges Datum (Format: YYYY-MM-DD)');
  }
...
function updateBericht($conn, $report_id) {
  ...
  try {
    ...
    if ($bericht['freigegeben']) {
      sendError(403, 'Freigegebene Berichte können nicht geändert werden');
    }
    ...
    if (isset($data['bemerkung'])) {
      ...
      $params[':bemerkung'] = $data['bemerkung'];
    }
...
function deleteBericht($conn, $report_id) {
  try {
    ...
    if ($bericht['freigegeben']) {
      sendError(403, 'Freigegebene Berichte können nicht gelöscht werden');
    }
...
function transformBerichtToFrontend($b) {
  return [
    ...
    'beschreibung' => $b['bemerkung'] ?? 'Keine Beschreibung',
    'zeitraum' => formatDate($b['arbeitsdatum']),
    'anzahl' => (int)ceil($b['arbeitszeit']),
    'auftragId' => (int)$b['auftrag_id'],
    'mitarbeiterId' => (int)$b['mitarbeiter_id'],
    'arbeitszeit' => (float)$b['arbeitszeit'],
    'arbeitsdatum' => $b['arbeitsdatum'],
    'freigegeben' => (bool)$b['freigegeben'],
    'auftragsStatus' => $b['auftrag_status'],
    'created_at' => $b['created_at'],
    'updated_at' => $b['updated_at'],
    // KUNDENDETAILS als Objekt
    'kunde' => [
      'id' => (int)$b['kunden_id'],
...
/**
 * Formatiert Datum für Frontend (dd.mm.yyyy)
 */
